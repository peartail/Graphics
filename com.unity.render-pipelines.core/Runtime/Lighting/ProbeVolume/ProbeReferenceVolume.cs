    ...
    public partial class ProbeReferenceVolume
    {
        ...
        void AddCell(Cell cell, int assetInstanceID)
        {
            ...
            if (!cells.TryGetValue(cell.index, out var cellInfo))
            {
                ...
                m_ToBeLoadedBlendingCells.Add(blendingCell);
            }
            else
            {
                cellInfo.referenceCount++;
        ...
        internal void RemovePendingAsset(ProbeVolumeAsset asset)
        {
            ...
            for (int i = m_ToBeLoadedCells.size - 1; i >= 0; i--)
            {
                ...
                {
                    if (m_ToBeLoadedCells[i].blendingCell.blending)
                        m_LoadedBlendingCells.Remove(m_ToBeLoadedCells[i].blendingCell);
                    else
                        m_ToBeLoadedBlendingCells.Remove(m_ToBeLoadedCells[i].blendingCell);
        ...
        bool AddBlendingBricks(BlendingCellInfo blendingCell)
        {
            ...
            for (int chunkIndex = 0; chunkIndex < chunkCount; chunkIndex += kTemporaryDataLocChunkCount)
            {
                ...
                if (bypassBlending)
                {
                    // Actually do the operation in AddBricks if cell is already loaded, because no index or chunk data must change,
                    // Only probe values need to be updated
                    // Ignore non uploaded cells, as they'll get uploaded by streaming.
                    if (!blendingCell.cellInfo.loaded) continue;
