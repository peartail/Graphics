    ...
    public partial class ProbeReferenceVolume
    {
        ...
        bool ComputeStreamingScoreForBlending(DynamicArray<BlendingCellInfo> cells, bool areUploaded)
        {
            ...
            for (int i = 0; i < cells.size; ++i)
            {
                ...
                bool fullyBlended = m_BakingStateLerpFactor >= 1.0f && cells[i].blendingFactor >= 1.0f;

                if (!cellInfo.loaded || fullyBlended)
                {
        ...
        {
            // TODO: this loop can probably be optimized, don't need to redo it all at each frame
            bool needsBlending = false;
            for (int i = 0; i < m_LoadedBlendingCells.size; i++)
            {
                var blendingCell = m_LoadedBlendingCells[i];
                // TODO: this assert may fail if a cell is not loaded and a blending cell is (remove from tobeloaded, check with bypass logic)
                ...
                for (int c = 0; c < blendingCell.chunkList.Count; c++)
                {
                    ...
                    m_BlendingPool.MapChunks(blendingCell.chunkList[c], dstIndex);
                    needsBlending = true;
                }
            }
